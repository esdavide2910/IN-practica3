...
def _():
    ...
    return StratifiedKFold, cs, np, optuna, pl, root_mean_squared_error
...
@app.cell
def _(cs, df_train):
    df_X_train_1 = df_train.select(cs.contains('density'))
    df_X_train_1
    return (df_X_train_1,)
...
@app.cell
def _(cs, df_test):
    df_X_test_1 = df_test.select(cs.contains('density'))
    df_X_test_1
    return (df_X_test_1,)
...
def _(mo):
    mo.md(r"""
    # 3. Entrenamiento e inferencia del modelo LightGBM
    """)
    return
...
def _(
    ...
    return lgb, study_LGBM


@app.cell
def _(study_LGBM):
    best_params_LGBM = study_LGBM.best_params
    print("Mejores hiperparámetros:", best_params_LGBM)
...
def _(X_test, X_train, best_params_LGBM, df_test, lgb, pl, y_train):
    ...
    results_test_LGBM.write_csv("submission/submission_1.csv")
    results_test_LGBM
    return
